...
router.get("/api/v1/name", validateApiKey, async (req, res) => {
  try {
    const name = (await db.get("name")) || "Dractyl Panel";
    res.json({ name });
  } catch (error) {
...
async function deleteInstance(instance) {
  try {
    await axios.get(
      `http://Dractyl:${instance.Node.apiKey}@${instance.Node.address}:${instance.Node.port}/instances/${instance.ContainerId}/delete`
    );
