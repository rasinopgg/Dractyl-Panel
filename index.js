 ...
 *
 *  Drayctyl Panel v1.0
 *  (c) 2026 MrDraynoX and contributors
 *
 */

/**
 * @fileoverview Main server file for Drayctyl Panel. Sets up the express application,
...
console.log(chalk.magenta(ascii) + chalk.cyan(`Drayctyl Panel v1.0\n`));
console.log(chalk.green(`Panel is running at: http://localhost:${config.port}`));
console.log(chalk.yellow(`Admin Login:`));
console.log(chalk.yellow(`  Username: ${chalk.white('admin')}`));
console.log(chalk.yellow(`  Password: ${chalk.white('password123')}`));
console.log('');

app.listen(config.port, () =>
  log.info(`Drayctyl is listening on port ${config.port}`)
);

app.get("*", async function (req, res) {
  res.render("errors/404", {
    req,
    name: (await db.get("name")) || "Drayctyl",
  });
});
